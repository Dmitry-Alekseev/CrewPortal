    ...
    Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface), modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            visibleDuties.forEach { duty ->
                val label = if (duty.dutyType == "FLIGHT") "${duty.flightNumber} ${duty.departureIata}-${duty.arrivalIata}" else if (duty.dutyType == "DEADHEAD") "PAX ${duty.flightNumber} ${duty.departureIata}-${duty.arrivalIata}" else if (duty.dutyType == "CREW_REST") "REST ${duty.departureIata}" else if (duty.dutyType == "TECHNICAL_STOP") "FUEL ${duty.departureIata}" else if (duty.dutyType == "OFF") { if (ru) "ВЫХОДНОЙ" else "OFF" } else if (duty.dutyType == "RESERVE") { if (ru) "РЕЗЕРВ" else "RESERVE" } else if (duty.dutyType == "STAY") { val city = AirportDatabase.cityName(duty.departureIata, duty.departureCity); if (ru) "ОТДЫХ В ${city.uppercase()}" else "Stay in $city" } else if (duty.dutyType in setOf("SIMULATOR", "MEDICAL", "SAFETY", "TRAINING", "EXAM")) duty.flightNumber else duty.dutyType
