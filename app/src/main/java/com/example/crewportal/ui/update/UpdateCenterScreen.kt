...
fun UpdateCenterScreen(
    ...
    Column(
        ...
        InfoCard(if (ru) "Журнал изменений — 3.0" else "Change log — 3.0") {
            listOf(
                if (ru) "Route Map использует общедоступную векторную карту OpenFreeMap без API-ключа." else "Route Map uses the public OpenFreeMap vector basemap without an API key.",
                if (ru) "Medical, Simulator и Line Check продлены на шесть месяцев и используют единый источник дат." else "Medical, Simulator and Line Check use one six-month qualification source.",
                if (ru) "Генератор резервирует связанные проверки до обычных рейсов и учитывает утверждённый отпуск." else "The generator reserves linked checks before ordinary flights and respects approved leave.",
                if (ru) "Время маршрута выбирается из min/max диапазона шагом пять минут и сохраняется в рейсе." else "Route time is selected from a five-minute min/max range and persisted on the leg.",
                if (ru) "Новый дизайн, планировщик Aircraft Delivery и одноразовый QA-генератор следующего месяца." else "New design, Aircraft Delivery planner and one-time next-month QA generator."
