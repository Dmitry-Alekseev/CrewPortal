...
fun UpdateCenterScreen(
    ...
    Column(
        ...
        InfoCard(if (ru) "Журнал изменений — 2.2.8" else "Change log — 2.2.8") {
            listOf(
                if (ru) "Во вкладке рейса добавлен заполняемый Electronic Pilot Logbook." else "Flight Details now includes a fillable Electronic Pilot Logbook.",
                if (ru) "Aircraft Delivery поддерживает A330neo, ручной HS- номер и добавление борта во Fleet после прибытия." else "Aircraft Delivery supports A330neo, manual HS- registration and post-arrival Fleet enrollment.",
                if (ru) "Leave request теперь сразу попадает в общий leave-state для Roster/Calendar/Payroll." else "Leave requests now apply to shared roster/calendar/payroll leave state.",
                if (ru) "Профиль обновлён на Line Pilot Instructor, лицензия от 19.06.2026." else "Profile updated to Line Pilot Instructor with license dated 19 June 2026.",
                if (ru) "В Operational Roster Change добавлена галка instructor/observer." else "Operational Roster Change can mark a duty as instructor/observer.",
                if (ru) "Instructor duty показывает Дмитрия третьим членом экипажа, не operating Captain." else "Instructor duties show Dmitrii as third crew member, not operating Captain.",
                if (ru) "Метаданные версии и workflow обновлены под 2.2.8." else "Version metadata and workflows updated to 2.2.8."
