...
fun UpdateCenterScreen(
    ...
    Column(
        ...
        InfoCard(if (ru) "Журнал изменений — 2.2.5" else "Change log — 2.2.5") {
            listOf(
                if (ru) "Destination в Operational Roster Change больше не подставляет Denpasar." else "Operational Roster Change destination no longer pre-fills Denpasar.",
                if (ru) "Список аэропортов отсортирован по ICAO и теперь листается." else "Airport suggestions are sorted by ICAO and scrollable.",
                if (ru) "Ручные маршруты берут город/аэропорт из общей базы, включая HKG." else "Manual routes resolve city/airport names from the shared database, including HKG.",
                if (ru) "Метаданные версии и workflow обновлены под 2.2.5." else "Version metadata and workflows updated to 2.2.5."
