...
fun UpdateCenterScreen(
    ...
    Column(
        ...
        InfoCard(if (ru) "Журнал изменений — 2.2.9" else "Change log — 2.2.9") {
            listOf(
                if (ru) "Route Map теперь полностью автономна и не обращается к заблокированным tile-серверам." else "Route Map is fully offline and no longer uses blocked tile servers.",
                if (ru) "LED и LHR используют long-haul block time из единого RouteCatalog." else "LED and LHR use authoritative long-haul block times from RouteCatalog.",
                if (ru) "Для неизвестного маршрута время оценивается по расстоянию, а не подставляется 2h 30m." else "Unknown-route time is distance-based instead of a fixed 2h 30m.",
                if (ru) "STAY показывает город назначения, а не generic crew hotel." else "STAY shows the destination city instead of generic crew-hotel text.",
                if (ru) "Метаданные версии и workflow обновлены под 2.2.9." else "Version metadata and workflows updated to 2.2.9."
