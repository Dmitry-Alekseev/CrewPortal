...
fun UpdateCenterScreen(
    ...
    Column(
        ...
        InfoCard(if (ru) "Журнал изменений — 2.2.1" else "Change log — 2.2.1") {
            listOf(
                if (ru) "Обновлена Airbus DB: A320/A321neo/A330/A350 и проверка типа борта." else "Airbus DB refreshed: A320/A321neo/A330/A350 and registration/type validation.",
                if (ru) "Генератор получил веса типов ВС по маршрутам и более разные времена вылетов." else "Generator now uses route aircraft weighting and more varied departure times.",
                if (ru) "Messages: новый стиль кнопок, английские даты, multi-select и нормальное удаление." else "Messages: cleaned buttons, English dates, multi-select and persistent deletion.",
                if (ru) "Скрытый 5-tap режим теперь умеет удалить черновик следующего месяца." else "Hidden 5-tap mode can now delete the next-month draft.",
                if (ru) "Метаданные версии и workflow обновлены под 2.2.1." else "Version metadata and workflows updated to 2.2.1."
