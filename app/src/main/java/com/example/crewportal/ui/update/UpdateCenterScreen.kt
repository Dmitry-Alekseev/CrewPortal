...
fun UpdateCenterScreen(
    ...
    Column(
        ...
        InfoCard(if (ru) "Журнал изменений — 3.0.2" else "Change log — 3.0.2") {
            listOf(
                if (ru) "Добавлен исторический парк A380 Thai Airways и type rating после экзамена 31 октября." else "Thai's historical A380 fleet and a post-check type rating were added.",
                if (ru) "Октябрь содержит два типа отпуска, перелёт в Тулузу, обучение и экзамены." else "October includes separated leave, Toulouse positioning, training and examinations.",
                if (ru) "Возвращение пассажиром и отдых защищают 1–3 ноября." else "Passenger return and recovery protect 1–3 November.",
                if (ru) "Добавлены направления США и проверка совместимости/дальности A380." else "US destinations and A380 airport/range checks were added.",
                if (ru) "В ручном изменении можно добавить OFF; Ташкент выбирается как обычный маршрут." else "Manual changes can add OFF; Tashkent is selected as an ordinary route."
