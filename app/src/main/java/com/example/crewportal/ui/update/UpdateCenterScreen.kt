...
fun UpdateCenterScreen(
    ...
    Column(
        ...
        InfoCard(if (ru) "Журнал изменений — 2.2.4" else "Change log — 2.2.4") {
            listOf(
                if (ru) "Текущий месяц больше не заменяется черновиком следующего месяца." else "Current month is no longer replaced by the next-month draft.",
                if (ru) "Roster и Calendar показывают следующий месяц только через явную кнопку preview." else "Roster and Calendar show next month only through an explicit preview button.",
                if (ru) "После генерации нового графика появляется сообщение компании." else "Generated roster drafts now create a company Messages item.",
                if (ru) "Тестовая генерация использует свежий random seed." else "Test generation uses a fresh random seed.",
                if (ru) "Метаданные версии и workflow обновлены под 2.2.4." else "Version metadata and workflows updated to 2.2.4."
