...
fun UpdateCenterScreen(
    ...
    Column(
        ...
        InfoCard(if (ru) "Журнал изменений — 2.1.10" else "Change log — 2.1.10") {
            listOf(
                if (ru) "Reserve отображается в Today’s Duty, включая ночные окна." else "Reserve appears in Today’s Duty, including night reserve windows.",
                if (ru) "Flight Details: порядок timeline, Open MEL и checklist исправлены." else "Flight Details: timeline order, Open MEL styling and checklist cleanup.",
                if (ru) "Weather показывает понятное сообщение при неверном ICAO." else "Weather shows friendly feedback for invalid ICAO codes.",
                if (ru) "Messages добавлены в нижнюю навигацию, Fleet перенесён в More." else "Messages added to bottom navigation; Fleet moved to More.",
                if (ru) "Добавлен workflow для автосборки APK и публикации Release." else "Added workflow for automatic APK build and Release publishing."
