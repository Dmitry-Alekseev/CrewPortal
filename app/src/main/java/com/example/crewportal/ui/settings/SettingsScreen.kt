    ...
    Column(
        ...
        Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface), modifier = Modifier.fillMaxWidth()) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Text(if (ru) "Пакет обновления: CrewPortal-2.0.2a.apk" else "Update package: CrewPortal-2.0.2a.apk", color = MaterialTheme.colorScheme.onSurfaceVariant)
        ...
        Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface), modifier = Modifier.fillMaxWidth()) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Text("Change Log — 2.0.2a", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
